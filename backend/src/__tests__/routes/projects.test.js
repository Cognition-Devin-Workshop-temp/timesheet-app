...
describe('Project Routes', () => {
  ...
  describe('POST /api/projects', () => {
    ...
    test('should create project without description and clientId', async () => {
      const newProject = { name: 'Minimal Project', startDate: '2024-01-01', status: 'active' };
      const createdProject = { id: 1, name: 'Minimal Project', description: null, client_id: null, start_date: '2024-01-01', status: 'active', client_name: null };
    ...
    test('should return 400 for missing name', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ startDate: '2024-01-01', status: 'active' });
    ...
    test('should return 400 for missing status', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Test Project', startDate: '2024-01-01' });
    ...
    test('should return 400 for invalid status value', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Test Project', startDate: '2024-01-01', status: 'invalid' });
    ...
    test('should return 400 for empty name', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ name: '', startDate: '2024-01-01', status: 'active' });
    ...
    test('should handle database insert error', async () => {
      ...
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Test Project', startDate: '2024-01-01', status: 'active' });

      expect(response.status).toBe(500);
      expect(response.body).toEqual({ error: 'Failed to create project' });
    });

    test('should handle database retrieval error after create', async () => {
      mockDb.run.mockImplementation(function(query, params, callback) {
        this.lastID = 1;
        callback.call(this, null);
      });
  ...
  describe('PUT /api/projects/:id', () => {
    test('should update project name', async () => {
      const updatedProject = { id: 1, name: 'Updated Name', description: 'Old Desc', client_id: 1, start_date: '2024-01-01', status: 'active', client_name: 'Client A' };
    ...
    test('should return 400 for invalid status value on update', async () => {
      const response = await request(app)
        .put('/api/projects/1')
    ...
    test('should handle database retrieval error after update', async () => {
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, { id: 1 });
      });
