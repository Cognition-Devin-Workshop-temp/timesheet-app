...
describe('Project Routes', () => {
  ...
  describe('GET /api/projects', () => {
    test('should return all projects for authenticated user', async () => {
      const mockProjects = [
        ...
        { id: 2, name: 'Project B', description: 'Desc B', client_id: 2, start_date: '2024-02-01', status: 'completed', client_name: 'Client B', created_at: '2024-01-02', updated_at: '2024-01-02' }
      ];
  ...
  describe('POST /api/projects', () => {
    test('should create new project with valid data', async () => {
      ...
      const createdProject = { id: 1, name: 'New Project', description: 'New Description', client_id: 1, start_date: '2024-01-01', status: 'active', client_name: 'Client A', created_at: '2024-01-01', updated_at: '2024-01-01' };
    ...
    test('should return 400 for missing name', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ clientId: 1, startDate: '2024-01-01', status: 'active' });
    ...
    test('should return 400 for missing clientId', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Test', startDate: '2024-01-01', status: 'active' });
    ...
    test('should return 400 for missing startDate', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Test', clientId: 1, status: 'active' });

      expect(response.status).toBe(400);
    });

    test('should handle database insert error', async () => {
      // Client ownership check passes
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, { id: 1 });
      });
      ...
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Test Project', clientId: 1, startDate: '2024-01-01', status: 'active' });
    ...
    test('should handle client not found', async () => {
      ...
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Test Project', clientId: 999, startDate: '2024-01-01', status: 'active' });

      expect(response.status).toBe(400);
      expect(response.body).toEqual({ error: 'Client not found or does not belong to user' });
    });

    test('should handle retrieval error after creation', async () => {
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, { id: 1 });
      });
    ...
    test('should handle database error during client check', async () => {
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(new Error('Database error'), null);
      });
  ...
  describe('PUT /api/projects/:id', () => {
    ...
    test('should update project clientId with client ownership check', async () => {
      // Project exists
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, { id: 1 });
      });
    ...
    test('should return 400 when updating clientId to non-existent client', async () => {
      // Project exists
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, { id: 1 });
      });
    ...
    test('should handle database error during client ownership check in update', async () => {
      // Project exists
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, { id: 1 });
      });

      // Client ownership check errors
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(new Error('Database error'), null);
      });
