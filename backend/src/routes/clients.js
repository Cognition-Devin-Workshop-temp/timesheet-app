...
router.get('/', (req, res) => {
  ...
  db.all(
    ...
    (err, rows) => {
      if (err) {
        logger.error({ err, requestId: req.id }, 'Database error fetching clients');
...
router.get('/:id', (req, res) => {
  ...
  db.get(
    ...
    (err, row) => {
      if (err) {
        logger.error({ err, requestId: req.id }, 'Database error fetching client');
        return res.status(500).json({ error: 'Internal server error' });
      }
...
router.post('/', (req, res, next) => {
  try {
    ...
    db.run(
      ...
      function(err) {
        if (err) {
          logger.error({ err, requestId: req.id }, 'Database error creating client');
        ...
        db.get(
          ...
          (err, row) => {
            if (err) {
              logger.error({ err, requestId: req.id }, 'Database error retrieving created client');
...
router.put('/:id', (req, res, next) => {
  try {
    ...
    db.get(
      ...
      (err, row) => {
        if (err) {
          logger.error({ err, requestId: req.id }, 'Database error checking client ownership');
          return res.status(500).json({ error: 'Internal server error' });
        }
        ...
        db.run(query, values, function(err) {
          if (err) {
            logger.error({ err, requestId: req.id }, 'Database error updating client');
          ...
          db.get(
            ...
            (err, row) => {
              if (err) {
                logger.error({ err, requestId: req.id }, 'Database error retrieving updated client');
...
router.delete('/', (req, res) => {
  ...
  db.run(
    ...
    function(err) {
      if (err) {
        logger.error({ err, requestId: req.id }, 'Database error deleting all clients');
...
router.delete('/:id', (req, res) => {
  ...
  db.get(
    ...
    (err, row) => {
      if (err) {
        logger.error({ err, requestId: req.id }, 'Database error checking client for deletion');
        return res.status(500).json({ error: 'Internal server error' });
      }
      ...
      db.run(
        ...
        function(err) {
          if (err) {
            logger.error({ err, requestId: req.id }, 'Database error deleting client');
