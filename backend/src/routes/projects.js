...
router.post('/', (req, res, next) => {
  try {
    ...
    db.run(
      `INSERT INTO projects (name, description, status, client_id, start_date, end_date, user_email)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [name, description || null, status || 'active', clientId || null, startDate || null, endDate || null, req.userEmail],
      function(err) {
        if (err) {
          console.error('Database error:', err);
