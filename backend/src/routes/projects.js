...
router.get('/', (req, res) => {
  ...
  db.all(
    `SELECT p.id, p.name, p.description, p.client_id, p.start_date, p.status, p.user_email, p.created_at, p.updated_at, c.name as client_name
     FROM projects p
...
router.post('/', (req, res, next) => {
  try {
    ...
    const proceed = (resolvedClientId) => {
      db.run(
        'INSERT INTO projects (name, description, client_id, start_date, status, user_email) VALUES (?, ?, ?, ?, ?, ?)',
        [name, description || null, resolvedClientId, startDate || null, status, req.userEmail],
        function(err) {
          if (err) {
            console.error('Database error:', err);
          ...
          // Return the created project with client join
          db.get(
            `SELECT p.id, p.name, p.description, p.client_id, p.start_date, p.status, p.user_email, p.created_at, p.updated_at, c.name as client_name
             FROM projects p
             LEFT JOIN clients c ON p.client_id = c.id
