...
router.get('/', (req, res) => {
  ...
  db.all(
    'SELECT p.id, p.name, p.description, p.client_id, p.start_date, p.status, p.created_at, p.updated_at, c.name as client_name FROM projects p LEFT JOIN clients c ON p.client_id = c.id WHERE p.user_email = ? ORDER BY p.name',
    [req.userEmail],
    (err, rows) => {
      if (err) {
