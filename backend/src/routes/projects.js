...
router.get('/', (req, res) => {
  ...
  let query = `
    SELECT p.id, p.name, p.description, p.status, p.client_id, p.start_date, p.end_date,
...
router.post('/', (req, res, next) => {
  try {
    ...
    const { name, description, status, clientId, startDate, endDate } = value;
    const db = getDatabase();
    ...
    db.get(
      ...
      (err, row) => {
        ...
        db.run(
          'INSERT INTO projects (name, description, status, client_id, user_email, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [name, description || null, status || 'active', clientId, req.userEmail, startDate || null, endDate || null],
          function(err) {
            if (err) {
              console.error('Database error:', err);
