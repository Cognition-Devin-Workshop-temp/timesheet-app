...
router.post('/', (req, res, next) => {
  try {
    ...
    // If clientId provided, verify client belongs to user
    if (clientId) {
      db.get(
...
function insertProject(db, name, description, clientId, startDate, status, userEmail, res) {
  db.run(
    ...
    [name, description || null, clientId, startDate || null, status || 'active', userEmail],
    function (err) {
      if (err) {
        console.error('Database error:', err);
  ...
  try {
    ...
    db.get(
      ...
      (err, row) => {
        ...
        // If clientId is being updated, verify client belongs to user
