...
export default function CallingPage({
  ...
  function getCallingRecord(attendeeId: string): CallingRecord | undefined {
    return session?.calling_records.find(
  ...
  const calledCount =
    session?.calling_records.filter(
      (r) => r.call_status !== "Not Called"
    ).length ?? 0;
  const willAttendCount =
    session?.calling_records.filter(
      (r) => r.call_status === "Will attend the session"
    ).length ?? 0;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="mb-6">
          <h1 className="text-xl sm:text-2xl font-bold text-deep-blue">
            Calling List
          </h1>
        ...
        <div className="space-y-4">
          {attendees.map((a) => {
            ...
            return (
              <div
                key={a._id}
                className="bg-white rounded-2xl shadow-sm border border-saffron/10 overflow-hidden"
              >
                {/* Header */}
                <div className="p-4 pb-3">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-deep-blue text-base">
                        {a.name}
                      </h3>
                ...
                <div className="px-4 pb-3">
                  ...
                  <select
                    value={currentStatus}
                    onChange={(e) =>
                      updateCallStatus(a._id, e.target.value)
                    }
                ...
                <div className="px-4 pb-4">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") addComment(a._id);
                      }}
                    />
                    <button
