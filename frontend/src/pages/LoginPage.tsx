...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isRegisterMode && error.response?.data?.error === 'Email already registered') {
        setError('This email is already registered. Please log in instead.');
      } else if (!isRegisterMode && error.response?.data?.error === 'Invalid email or password') {
      ...
      <Paper elevation={3} sx={{ padding: 3, width: '100%', maxWidth: 500 }}>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <TextField
            ...
            disabled={isLoading}
            helperText={isRegisterMode ? 'Minimum 8 characters' : undefined}
          />
          <Button
            type="submit"
            fullWidth
