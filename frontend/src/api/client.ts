...
class ApiClient {
  ...
  async createProject(data: { name: string; description?: string; status?: string; clientId: number; startDate?: string | null; endDate?: string | null }) {
    const response = await this.client.post('/api/projects', data);
    return response.data;
  }

  async updateProject(id: number, data: { name?: string; description?: string; status?: string; clientId?: number; startDate?: string | null; endDate?: string | null }) {
